...
void visit_neighbours(int start)
{
	if(visited[start]) 
	{
		return;
	...
	visited[start] = true;
	for(auto neigh : roads[start])
	{
		visit_neighbours(neigh);
	}
}
...
int main()
{
	...
	for(int t=0; t<test_cases; t++)
	{
		...
		for(int i=0; i<M; i++)
		{
			ll field1, field2;
			cin>>field1>>field2;
			roads[field1].push_back(field2);
			roads[field2].push_back(field1);
		}
		ll disconected_components = 0;
		for(int i=1; i<=N; i++)
		{
			if(!visited[i])
			{
				visit_neighbours(i);
				disconected_components++;
			}
		}
		int min_distance = disconected_components - 1;
		cout << min_distance << "\n";
	}
	return 0;
}
